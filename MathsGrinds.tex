...
\coordinate [label=right:$Point$] (B) at (4.5, 2);
...
\begin{tikzpicture}
\coordinate [label=below:$Center$] (A) at (2,2);
\coordinate [label=right:$Point\;1$] (B) at (4.5, 2);
\coordinate [label=right:$Point\;2$] (C) at (3.75, 3.75);
\draw (2,2) circle (2.5cm);
\draw (A) -- node[above] {$Radius$} (B);
\draw (A) -- (C);
...
\begin{itemize}
\item Probability of Event A \textbf{AND} Event B =  P(E) * P(B)
\item Probability of Event A \textbf{OR} Event B =  P(E) + P(B)
\end{itemize}

\textbf{e.g}
If you throw two dice (Dice A and Dice B) at the same time, what are the chances (probability) of the number 6 been shown on (1) both the dice A AND dice B and (2) or either dice A OR dice B.

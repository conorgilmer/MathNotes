...
\newpage
\textbf{Example 30 60 90 degree triangle}

\begin{tikzpicture}[scale=1.25]%,cap=round,>=latex]

\coordinate [label=left:$A$] (D) at (-.75cm,-1.0cm);
\coordinate [label=right:$B$] (G) at (1.0cm,-1.0cm);
\coordinate [label=above:$C$] (E) at (1.0cm,1.0cm);
\draw (D) -- node[above] {$2$} (E) -- node[right] {$\sqrt{3}$} (G) -- node[below] {$1$} (D);

\draw (1.0cm,-1.0cm) rectangle (0.75cm,-0.75cm);
...
\coordinate [label=left:$D$] (D) at (-.75cm,-1.0cm);
\coordinate [label=right:$G$] (G) at (1.0cm,-1.0cm);
\coordinate [label=above:$E$] (E) at (1.0cm,1.0cm);
\draw (D) -- node[above] {$6$} (E) -- node[right] {$3\sqrt{3}$} (G) -- node[below] {$3$} (D);
